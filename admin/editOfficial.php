...
$old_province = $row_check_offical['province'];
$old_zip = $row_check_offical['zip'];
$old_city = $row_check_offical['city'];
...
$stmt_official->close();
  ...
  if($_POST['edit_province_check'] == 'true' || $_POST['edit_province_check'] === TRUE){
    ...
    $date_activity = $now = date("j-n-Y g:i A");  
    $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL PROVINCE - '.' ' .$official_id.' |' .' '. ' FROM '.$old_province.' TO '. $edit_province;
    $status_activity_log = 'update';
    $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
    $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
    $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
  ...
  if($_POST['edit_city_check'] == 'true' || $_POST['edit_city_check'] === TRUE){
    ...
    $date_activity = $now = date("j-n-Y g:i A");  
    $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL CITY - '.' ' .$official_id.' |' .' '. ' FROM '.$old_city.' TO '. $edit_city;
    $status_activity_log = 'update';
    $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
    $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
    $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
