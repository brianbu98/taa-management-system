...
try{
  ...
  if(isset($_REQUEST['edit_residence_id'])){
    ...
    $sql_incident_check = "SELECT incident_record.*, incident_status.*, incident_complainant.*, incident_record.incidentlog_id AS comp FROM `incident_record` 
    INNER JOIN incident_complainant ON incident_record.incidentlog_id = incident_complainant.incident_main 
    INNER JOIN incident_status ON incident_record.incidentlog_id = incident_status.incident_main 
    WHERE person_id = ? OR complainant_id = ? GROUP BY incident_record.incidentlog_id";
    ...
    while($row_incident_check = $result_incident_check->fetch_assoc()){
      ...
      if($row_incident_check['complainant_id'] == $edit_residence_id){
        $color = 1;
        $delete_record = '<i style="cursor: pointer;  color: red;  text-shadow: -1px 0 black, 0 1px black, 1px 0 black, 0 -1px black;" class="fa fa-times text-lg px-2 deleteRecordComplainant" data-id="'.$row_incident_check['complainant_id'].'" id="'.$row_incident_check['incident_main'].'" ></i>';
      }else{
        $color = 2;
        $delete_record = '<i style="cursor: pointer;  color: red;  text-shadow: -1px 0 black, 0 1px black, 1px 0 black, 0 -1px black;" class="fa fa-times text-lg px-2 deleteRecordPerson" data-id="'.$row_incident_check['person_id'].'" id="'.$row_incident_check['incident_main'].'"></i>';
      }
      ...
      $subdata[] = $color;
      $subdata[] = $row_incident_check['comp'];
      ...
      // $subdata[] =   '<i style="cursor: pointer;  color: yellow;  text-shadow: -1px 0 black, 0 1px black, 1px 0 black, 0 -1px black;" class="fa fa-book-open text-lg px-2 viewRecord" id="'.$row_incident_check['comp'].'"></i>
      // '.$delete_record.'';
